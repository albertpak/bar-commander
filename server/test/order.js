...
describe('Orders', () => {
    ...
    describe('CRUD', () => {
        ...
        it('POST /orders should create new order', async () => {
            const res = await chai.request(server).post('/orders').send({
                tableNumber: 102,
                date: Date.now(),
                listProduct: [],
                totalPrice: 11,
            ...
            res.body.should.be.a('object');
            res.body.data.should.have.property('tableNumber').eql('102');
        });

        it(`PUT /orders/:id should PUT the order with :id`, async () => {
        ...
        it(`DELETE /orders/:id should DELETE the order with :id`, async () => {
